    ...
    <div className="fixed inset-0 w-screen h-screen bg-black overflow-hidden font-sans text-white select-none">
      ...
      <AnimatePresence>
        {gameState === 'playing' && (
          <>
            {/* Chat UI */}
            <div className={`absolute bottom-24 left-6 z-[100] w-full max-w-[300px] pointer-events-none flex flex-col gap-2 ${isMobile ? 'scale-75 origin-bottom-left' : ''}`}>
            ...
            {hudData.roundWinner && !hudData.winner && (
              ...
              >
                ...
                >
                  <h2 className={`text-6xl font-black italic tracking-tighter drop-shadow-2xl ${hudData.roundWinner === 'blue' ? 'text-blue-400' : 'text-red-400'}`}>
            ...
            {countdown > 0 && (
              <div className="absolute inset-0 flex items-center justify-center z-50 pointer-events-none bg-black/40">
                <motion.div 
                  ...
                  className="text-center"
                >
                  <h1 className="text-[12rem] font-black italic text-white drop-shadow-[0_0_50px_rgba(255,255,255,0.8)] stroke-black stroke-2">
                    {countdown}
                  </h1>
                  <p className="text-4xl font-bold uppercase tracking-[1em] text-white/80 mt-4 animate-pulse">Get Ready</p>
                </motion.div>
              </div>
            )}
      ...
      <AnimatePresence>
        {gameState === 'menu' && (
          <motion.div 
            ...
            animate={{ opacity: 1 }}
            exit={{ opacity: 0 }}
            className={`absolute inset-0 z-50 flex bg-zinc-950 overflow-y-auto ${isMobile ? 'flex-col' : 'flex-row'}`}
          >
            {/* Split Layout: Left Side (Branding & Profile) */}
            <div className={`relative flex-1 flex flex-col justify-between p-6 md:p-16 border-r border-white/5 ${isMobile ? 'min-h-screen' : ''}`}>
              ...
              <div className="relative z-10">
                ...
                <h1 className={`${isMobile ? 'text-6xl' : 'text-[120px]'} font-black tracking-tighter leading-[0.85] uppercase italic text-white mb-6`}>
                  Poly<br/><span className="text-emerald-500">Dodge</span>
                </h1>
                <p className="text-white/40 max-w-sm font-medium leading-relaxed text-xs md:text-base">
                  The ultimate low-poly competitive dodgeball arena. Aim, dodge, and dominate the leaderboard in high-stakes 4v4 matches.
                </p>
              </div>

              {/* Profile Section */}
              <div className={`relative z-10 flex ${isMobile ? 'flex-col items-stretch' : 'items-end'} gap-4 md:gap-6 mt-8 md:mt-0`}>
                <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-[24px] md:rounded-[32px] p-4 md:p-6 flex items-center gap-4 md:gap-6 shadow-2xl">
                  <div className="relative shrink-0">
                    <div className="w-12 h-12 md:w-16 md:h-16 rounded-xl md:rounded-2xl bg-emerald-500/20 flex items-center justify-center border border-emerald-500/30 overflow-hidden">
                      <span className="font-black text-emerald-400 text-xl md:text-2xl">{playerProfile.level}</span>
                    </div>
                    <div className="absolute -bottom-1 -right-1 bg-emerald-500 text-black text-[8px] md:text-[10px] font-black px-1.5 md:px-2 py-0.5 rounded-full shadow-lg">LVL</div>
                  </div>
                  <div className="flex-1 min-w-0">
                    <input 
                      ...
                      className="bg-transparent font-black text-white focus:outline-none border-b-2 border-transparent focus:border-emerald-500/50 transition-all w-full text-xl md:text-2xl mb-1"
                      placeholder="Enter Name"
                    />
                    <div className="flex items-center gap-3">
                      <span className="text-[8px] md:text-[10px] font-black text-emerald-400 uppercase tracking-widest truncate">{playerProfile.rank}</span>
                ...
                <div className="bg-zinc-900/50 backdrop-blur-xl border border-white/10 rounded-[24px] md:rounded-[32px] p-4 md:p-6 flex items-center gap-4 shadow-2xl">
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full bg-amber-400 shadow-[0_0_20px_rgba(251,191,36,0.4)] flex items-center justify-center shrink-0">
                    <div className="w-3 h-3 md:w-4 md:h-4 rounded-full border-2 border-amber-600/30" />
                  </div>
                  <div className="flex flex-col">
                    <span className="text-[8px] md:text-[10px] font-black text-white/30 uppercase tracking-widest leading-none mb-1">Credits</span>
                    <span className="font-mono font-black text-amber-400 text-xl md:text-2xl leading-none">{playerProfile.coins.toLocaleString()}</span>
                  </div>
                </div>
              </div>
            </div>
            ...
            <div className={`relative flex-1 bg-zinc-900/20 flex flex-col justify-center p-6 md:p-16 ${isMobile ? 'pb-24' : ''}`}>
              ...
              <div className={`w-full ${isMobile ? 'max-w-md mx-auto' : 'max-w-md'} space-y-4 md:space-y-6 relative z-10`}>
                ...
                <button 
                  ...
                  className="w-full group relative overflow-hidden bg-white text-black px-6 md:px-8 py-6 md:py-8 rounded-[24px] md:rounded-[32px] font-black flex items-center justify-between transition-all hover:scale-[1.02] active:scale-95 shadow-2xl"
                >
                  <div className="flex items-center gap-4 md:gap-6 relative z-10">
                    <div className="w-10 h-10 md:w-14 md:h-14 rounded-xl md:rounded-2xl bg-black flex items-center justify-center group-hover:rotate-12 transition-transform">
                      <Globe className="w-6 h-6 md:w-8 md:h-8 text-emerald-400" />
                    </div>
                    <div className="flex flex-col items-start">
                      <span className="text-xl md:text-2xl tracking-tighter">RANKED MATCH</span>
                      <span className="text-[8px] md:text-[10px] font-bold opacity-40 tracking-widest uppercase">Competitive 4v4 Arena</span>
                    </div>
                  </div>
                  <div className="w-8 h-8 md:w-10 md:h-10 rounded-full border-2 border-black/10 flex items-center justify-center group-hover:bg-black group-hover:text-white transition-all">
                ...
                <div className="grid grid-cols-2 gap-3 md:gap-4">
                  <button 
                    ...
                    className="group bg-zinc-900/50 hover:bg-zinc-800 backdrop-blur-md border border-white/5 hover:border-white/20 p-4 md:p-6 rounded-[24px] md:rounded-[32px] flex flex-col items-center gap-2 md:gap-3 transition-all active:scale-95"
                  >
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-blue-500/10 flex items-center justify-center group-hover:scale-110 transition-transform">
                      <Users className="w-5 h-5 md:w-6 md:h-6 text-blue-400" />
                    </div>
                    <span className="font-black uppercase tracking-widest text-[10px] md:text-xs">Casual</span>
                  </button>
                  ...
                  <button 
                    ...
                    className="group bg-zinc-900/50 hover:bg-zinc-800 backdrop-blur-md border border-white/5 hover:border-white/20 p-4 md:p-6 rounded-[24px] md:rounded-[32px] flex flex-col items-center gap-2 md:gap-3 transition-all active:scale-95"
                  >
                    <div className="w-10 h-10 md:w-12 md:h-12 rounded-xl md:rounded-2xl bg-white/5 flex items-center justify-center group-hover:scale-110 transition-transform">
                      <Play className="w-5 h-5 md:w-6 md:h-6 text-white/60" />
                    </div>
                    <span className="font-black uppercase tracking-widest text-[10px] md:text-xs">Practice</span>
                  </button>
                </div>
        ...
        {matchmakingState !== 'none' && (
          ...
          >
            <motion.div 
              initial={{ scale: 0.9, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              className="w-full max-w-4xl bg-zinc-900 border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl flex flex-col"
            >
              <div className="p-12 text-center border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent">
                <div className="flex items-center justify-center gap-3 mb-6">
                  ...
                  <span className="text-emerald-500 font-black tracking-[0.5em] uppercase text-xs">
                    {matchmakingState === 'searching' ? 'Global Matchmaking' : 'Battle Lobby'}
                  </span>
                  <div className="w-12 h-1 bg-emerald-500" />
                </div>
                
                <h2 className="text-6xl md:text-8xl font-black italic uppercase tracking-tighter text-white leading-none mb-4">
                  {matchmakingState === 'searching' ? 'Searching...' : 'Ready Up'}
                </h2>
                
                <div className="text-white/30 font-black uppercase tracking-[0.3em] text-sm">
              ...
              <div className="p-12 flex flex-col items-center gap-12">
                {matchmakingState === 'searching' && !matchRoom && (
                  <div className="flex flex-col items-center">
                    ...
                    <div className="text-2xl font-black text-white uppercase italic tracking-tight mb-2">Establishing Connection</div>
                    <div className="text-white/40 text-xs font-bold uppercase tracking-widest">Scanning for available servers...</div>
                  </div>
                )}
                  ...
                  <div className="w-full space-y-12">
                    <div className="grid grid-cols-2 gap-12">
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-blue-500/30 pb-4">
                          <span className="text-blue-400 font-black uppercase tracking-widest text-sm italic">Blue Team</span>
                          <span className="text-white/20 text-[10px] font-black uppercase">Alpha Squad</span>
                        </div>
                        <div className="grid grid-cols-1 gap-3">
                          {matchRoom.players.filter((p: any) => p.team === 'blue').map((p: any) => (
                            <div key={p.id} className="bg-blue-500/5 border border-blue-500/10 p-4 rounded-2xl flex items-center justify-between">
                              <span className="text-white font-bold">{p.id === socket?.id ? p.name : 'Agent'}</span>
                              <div className="w-2 h-2 rounded-full bg-blue-400 shadow-[0_0_10px_rgba(96,165,250,0.5)]" />
                            </div>
                          ))}
                          {Array.from({ length: 4 - matchRoom.players.filter((p: any) => p.team === 'blue').length }).map((_, i) => (
                            <div key={i} className="bg-white/5 border border-dashed border-white/10 p-4 rounded-2xl flex items-center justify-center">
                              <span className="text-white/10 text-[10px] font-black uppercase tracking-widest">Waiting...</span>
                            </div>
                          ))}
                        </div>
                      </div>
                      ...
                      <div className="space-y-6">
                        <div className="flex items-center justify-between border-b border-red-500/30 pb-4">
                          <span className="text-red-400 font-black uppercase tracking-widest text-sm italic">Red Team</span>
                          <span className="text-white/20 text-[10px] font-black uppercase">Omega Squad</span>
                        </div>
                        <div className="grid grid-cols-1 gap-3">
                          {matchRoom.players.filter((p: any) => p.team === 'red').map((p: any) => (
                            <div key={p.id} className="bg-red-500/5 border border-red-500/10 p-4 rounded-2xl flex items-center justify-between">
                              <span className="text-white font-bold">{p.id === socket?.id ? p.name : 'Agent'}</span>
                              <div className="w-2 h-2 rounded-full bg-red-400 shadow-[0_0_10px_rgba(248,113,113,0.5)]" />
                            </div>
                          ))}
                          {Array.from({ length: 4 - matchRoom.players.filter((p: any) => p.team === 'red').length }).map((_, i) => (
                            <div key={i} className="bg-white/5 border border-dashed border-white/10 p-4 rounded-2xl flex items-center justify-center">
                              <span className="text-white/10 text-[10px] font-black uppercase tracking-widest">Waiting...</span>
                            </div>
                          ))}
                    ...
                    <div className="bg-white/5 border border-white/10 p-8 rounded-[2.5rem] flex items-center justify-between">
                      <div className="flex items-center gap-6">
                        ...
                        <div>
                          <div className="text-[10px] font-black text-white/30 uppercase tracking-widest mb-1">Match Rules</div>
                          <div className="text-2xl font-black text-white uppercase italic">First to 4 Round Wins</div>
                        </div>
                      </div>
                      
                      {matchRoom.state === 'ready_check' && (
                        <div className="text-right">
                          <div className="text-[10px] font-black text-emerald-400 uppercase tracking-widest mb-1">Commencing In</div>
                          <div className="text-4xl font-black text-white font-mono">{matchRoom.timer}s</div>
                        </div>
                      )}
                    </div>
                  </div>
                ...
                <button 
                  ...
                  className="px-16 py-5 bg-white/5 hover:bg-red-500/20 text-white/40 hover:text-red-400 border border-white/10 hover:border-red-500/30 rounded-2xl font-black uppercase tracking-[0.3em] transition-all active:scale-95 text-xs"
                >
                  Abort Mission
                </button>
              </div>
      ...
      {showGameEnd && (
        <div className="fixed inset-0 z-[9999] bg-black/95 backdrop-blur-xl flex items-center justify-center p-4 font-sans">
          <div className="w-full max-w-5xl bg-zinc-900 border border-white/10 rounded-3xl overflow-hidden shadow-2xl flex flex-col max-h-[90vh] animate-in fade-in zoom-in duration-300">
            {/* Header */}
              <div className={`p-8 md:p-10 text-center ${hudData.winner === 'blue' ? 'bg-blue-900/20' : 'bg-red-900/20'} border-b border-white/5 relative overflow-hidden`}>
                <div className={`absolute inset-0 opacity-10 ${hudData.winner === 'blue' ? 'bg-blue-500' : 'bg-red-500'} blur-3xl`} />
                <h2 className={`relative z-10 text-5xl md:text-7xl font-black uppercase italic tracking-tighter ${hudData.winner === 'blue' ? 'text-blue-400' : 'text-red-400'}`}>
              ...
              {/* Scoreboard Content */}
              <div className="flex-1 overflow-y-auto p-6 md:p-8 space-y-8 custom-scrollbar bg-zinc-950/50">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  {['blue', 'red'].map(team => (
                    <div key={team} className="space-y-4">
                      <div className="flex items-center justify-between border-b border-white/5 pb-2">
                        <h3 className={`text-xl font-black uppercase tracking-widest ${team === 'blue' ? 'text-blue-400' : 'text-red-400'}`}>
                          {team} Team
                        </h3>
                        <div className={`px-2 py-0.5 rounded text-[10px] font-bold uppercase ${team === 'blue' ? 'bg-blue-500/10 text-blue-400' : 'bg-red-500/10 text-red-400'}`}>
                      ...
                      <div className="space-y-2">
                        {(hudData.scoreboard || [])
                          ...
                          .map(player => (
                            <div key={player.id} className="bg-white/5 p-3 rounded-xl flex items-center justify-between group hover:bg-white/10 transition-colors border border-transparent hover:border-white/5">
                              <div className="flex items-center gap-3">
                                <div className={`w-10 h-10 rounded-lg flex items-center justify-center font-black text-lg ${team === 'blue' ? 'bg-blue-500/20 text-blue-400' : 'bg-red-500/20 text-red-400'}`}>
                                  {player.name[0]}
                                </div>
                                <div>
                                  <div className="font-bold text-white flex items-center gap-2">
                                    {player.name}
                                    {player.id === socket?.id && <span className="text-[9px] bg-emerald-500 text-black px-1.5 py-0.5 rounded-full font-black tracking-wide">YOU</span>}
                                  </div>
                              ...
                              <div className="flex gap-4 text-right">
                                 <div className="flex flex-col items-end w-12">
                                   <span className="text-white font-black text-lg leading-none">{player.stats.eliminations}</span>
                                   <span className="text-[8px] uppercase text-white/30 font-bold tracking-wider">Kills</span>
                                 </div>
                                 <div className="flex flex-col items-end w-12">
                                   <span className="text-emerald-400 font-black text-lg leading-none">{player.stats.catches}</span>
                                   <span className="text-[8px] uppercase text-emerald-400/50 font-bold tracking-wider">Catch</span>
                                 </div>
                                 <div className="flex flex-col items-end w-12">
                                   <span className="text-blue-400 font-black text-lg leading-none">{player.stats.blocks}</span>
                                   <span className="text-[8px] uppercase text-blue-400/50 font-bold tracking-wider">Block</span>
                                 </div>
                              </div>
                            </div>
                          ))}
                          
                          {(hudData.scoreboard || []).filter(p => p.team === team).length === 0 && (
                            <div className="text-white/20 text-sm italic py-8 text-center border border-dashed border-white/5 rounded-xl">
              ...
              {/* Footer Actions */}
              <div className="p-6 md:p-8 bg-zinc-900 border-t border-white/5 flex flex-col md:flex-row gap-4 justify-center items-center shrink-0">
        ...
        {showCustomization && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="w-full max-w-5xl bg-zinc-900 border border-white/10 rounded-[2.5rem] overflow-hidden flex flex-col h-[85vh] shadow-[0_0_100px_rgba(0,0,0,0.5)]"
            >
              {/* Header */}
              <div className="p-8 md:p-12 border-b border-white/5 flex items-end justify-between bg-gradient-to-b from-white/5 to-transparent">
                <div>
                ...
                <button 
                  ...
                  className="w-16 h-16 rounded-full bg-white/5 hover:bg-white/10 flex items-center justify-center transition-all active:scale-90 group"
                >
              ...
              {/* Tabs */}
              <div className="flex px-8 md:px-12 gap-8 border-b border-white/5">
                {(['Balls'] as const).map((tab) => (
                  <button 
                    key={tab} 
                    onClick={() => setCustomizationTab(tab)}
                    className={`py-6 font-black uppercase tracking-[0.3em] text-xs transition-all relative ${
              ...
              {/* Grid Content */}
              <div className="flex-1 overflow-y-auto p-8 md:p-12 custom-scrollbar bg-zinc-950/20">
                  ...
                  {customizationTab === 'Balls' && balls.map(ball => {
                    ...
                    return (
                      <button 
                        ...
                        className={`group relative p-6 rounded-[2rem] border transition-all flex flex-col gap-6 ${
                          selectedBall === ball 
                      ...
                      >
                        <div className="flex justify-between items-start">
                          <div className={`w-20 h-20 rounded-full border-4 flex items-center justify-center transition-transform group-hover:scale-110 ${
                            ...
                            {!isUnlocked && <Lock className="w-8 h-8 text-white/40 absolute" />}
                            <div className="w-10 h-10 rounded-full bg-white/10 border border-white/20 blur-[1px]" />
                          </div>
                          {selectedBall === ball ? (
                            <div className="bg-emerald-500 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Equipped</div>
                          ) : !isUnlocked ? (
                            <div className="flex items-center gap-2 bg-amber-500 text-black text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">
                              <div className="w-2 h-2 rounded-full bg-amber-700/30" />
                              {price}
                            </div>
                          ) : (
                            <div className="bg-white/10 text-white/40 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest">Unlocked</div>
                          )}
                        </div>
                        
                        <div className="text-left">
                          <div className="text-2xl font-black text-white italic uppercase tracking-tighter mb-1">{ball}</div>
                          <div className="text-[10px] font-bold text-white/30 uppercase tracking-widest">
                            {isUnlocked ? 'Standard Issue Equipment' : 'Restricted Access'}
                          </div>
                        </div>
        ...
        {showInGameMenu && (
          ...
          >
            <motion.div 
              ...
              animate={{ scale: 1, y: 0 }}
              className="w-full max-w-sm bg-zinc-900 border border-white/10 rounded-[3rem] overflow-hidden shadow-[0_0_100px_rgba(0,0,0,0.8)]"
            >
              <div className="p-12 text-center bg-gradient-to-b from-white/5 to-transparent">
                <div className="flex items-center justify-center gap-3 mb-6">
                ...
                <h2 className="text-6xl font-black italic uppercase tracking-tighter text-white leading-none mb-2">Tactical</h2>
                <h2 className="text-4xl font-black italic uppercase tracking-tighter text-emerald-500 leading-none">Intermission</h2>
              </div>
              
              <div className="p-8 space-y-4">
                <button 
                  onClick={() => setShowInGameMenu(false)}
                  className="w-full bg-emerald-500 text-black py-6 rounded-2xl font-black uppercase tracking-[0.4em] hover:bg-emerald-400 transition-all active:scale-95 shadow-2xl flex items-center justify-center gap-3"
                >
                ...
                <button 
                  ...
                  className="w-full bg-white/5 text-white py-6 rounded-2xl font-black uppercase tracking-[0.4em] hover:bg-white/10 transition-all active:scale-95 border border-white/5 flex items-center justify-center gap-3"
                >
                ...
                <div className="pt-4 mt-4 border-t border-white/5">
                  <button 
                    ...
                    className="w-full bg-red-500/10 text-red-500 py-6 rounded-2xl font-black uppercase tracking-[0.4em] hover:bg-red-500/20 transition-all active:scale-95 border border-red-500/10 flex items-center justify-center gap-3"
                  >
                    <X className="w-5 h-5" />
                    Abort Mission
                  </button>
            ...
            >
              ...
              <div className="p-8 bg-zinc-950/50 border-t border-white/5">
                <button 
                  ...
                  className="w-full bg-white text-black py-5 rounded-2xl font-black uppercase tracking-[0.3em] hover:bg-emerald-400 transition-all active:scale-95 shadow-2xl"
                >
                  Apply & Synchronize
                </button>
              </div>
        ...
        {showInstructions && (
          ...
          >
            <motion.div 
              initial={{ scale: 0.95, y: 20 }}
              animate={{ scale: 1, y: 0 }}
              className="w-full max-w-4xl bg-zinc-900 border border-white/10 rounded-[3rem] overflow-hidden shadow-2xl flex flex-col max-h-[90vh]"
            >
              <div className="p-12 border-b border-white/5 bg-gradient-to-b from-white/5 to-transparent flex justify-between items-end">
                <div>
                  <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex-1 overflow-y-auto p-12 custom-scrollbar bg-zinc-950/20">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                  <section>
                    <h3 className="text-amber-500 font-black uppercase tracking-[0.3em] text-xs mb-8 flex items-center gap-3">
                      <div className="w-2 h-2 rounded-full bg-amber-500" />
                      Core Mechanics
                    </h3>
                    <div className="space-y-6">
                      {[
                        { title: 'Movement', desc: 'WASD or Joystick to navigate the arena. Use your environment for cover.', icon: Globe },
                        { title: 'Combat', desc: 'Left Click or Fire button to throw. Hold to charge power and speed.', icon: Target },
                        { title: 'Defense', desc: 'Right Click or Block button to deflect incoming projectiles.', icon: Shield },
                        { title: 'Recovery', desc: 'Walk over idle balls to replenish your arsenal.', icon: Zap }
                      ].map((item, i) => (
                        <div key={i} className="flex gap-6 group">
                          <div className="w-12 h-12 rounded-2xl bg-white/5 flex items-center justify-center group-hover:bg-amber-500/10 transition-colors shrink-0">
                            <item.icon className="w-6 h-6 text-white/40 group-hover:text-amber-500 transition-colors" />
                          </div>
                          <div>
                            <div className="text-white font-black uppercase italic tracking-tight text-lg mb-1">{item.title}</div>
                  ...
                  <section>
                    <h3 className="text-emerald-500 font-black uppercase tracking-[0.3em] text-xs mb-8 flex items-center gap-3">
                    ...
                    <div className="bg-white/5 rounded-[2rem] p-8 border border-white/5 space-y-6">
                      <div className="flex items-center gap-4">
                        ...
                        <div className="text-white font-black uppercase italic tracking-tight text-xl">Elimination</div>
                      </div>
                      <p className="text-white/40 text-sm leading-relaxed">
                        The objective is simple: eliminate the opposing team. A team wins the round when all enemy players are out.
                      </p>
                      <div className="pt-6 border-t border-white/5">
                        <div className="text-[10px] font-black text-white/20 uppercase tracking-widest mb-4">Match Format</div>
                        <div className="flex justify-between items-center">
                          <span className="text-white font-bold">First to 4 Rounds</span>
                          <span className="text-emerald-500 font-black font-mono">VICTORY</span>
                        </div>
                      </div>
                    </div>
                  </section>
              ...
              <div className="p-8 border-t border-white/5 bg-zinc-900 flex justify-center">
                <button 
                  onClick={() => setShowInstructions(false)}
                  className="px-16 py-5 bg-white text-black rounded-2xl font-black uppercase tracking-[0.4em] hover:bg-amber-500 transition-all active:scale-95 shadow-2xl"
                >
                  Understood
                </button>
              </div>
