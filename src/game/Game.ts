...
export class Game {
  ...
  private fpShield: THREE.Mesh | null = null;
  ...
  private lastSyncedRotation: number = 0;
  ...
  private setupFPShield() {
    ...
    this.fpShield = new THREE.Mesh(geometry, material);
    this.fpShield.position.set(0, 0, -0.5);
    this.fpShield.visible = false;
    this.camera.add(this.fpShield);
    this.scene.add(this.camera); // Ensure camera is in scene if not already
  }
  ...
  public sendEmote(emote: string) {
    this.showEmote(this.myId || 'local', emote);
    if (this.socket) {
      this.socket.emit('player_emote', { emote });
    }
  }

  private showEmote(playerId: string, text: string) {
    const player = this.players.get(playerId);
    if (!player) return;

    // Remove existing emote for this player
    if (this.emoteSprites.has(playerId)) {
      const oldSprite = this.emoteSprites.get(playerId)!;
      this.scene.remove(oldSprite);
      this.emoteSprites.delete(playerId);
    }

    // Create canvas for emote
    ...
    this.emoteSprites.set(playerId, sprite);
    this.scene.add(sprite);

    // Auto-remove after 3 seconds
    setTimeout(() => {
      if (this.emoteSprites.get(playerId) === sprite) {
  ...
  private tryGrabBall() {
    ...
    if (closestBallId !== null) {
      ...
      if (ball) {
        const playerId = this.myId || 'local';
        this.onBallGrabbed(closestBallId, playerId);
        sounds.playPickup();
        if (this.socket) {
          this.socket.emit('ball_grab', { ballId: closestBallId });
        }
      }
    }
  }
  ...
  private throwBall(ballId: number, charge: number = 0) {
    ...
    let accuracy = 1.0;
    if (this.moveForward || this.moveBackward || this.moveLeft || this.moveRight) accuracy = 0.95;
    ...
    let power = 25 + (charge * 35); // Range 25 to 77.5
  ...
  public startOnline(room: any, socket: Socket) {
    ...
    this.onUpdateHUD({ 
      winner: null, 
      timer: 180,
      blueWins: room.blueWins || 0,
      redWins: room.redWins || 0,
      isOut: false,
      bluePlayersLeft: 4,
      redPlayersLeft: 4,
    ...
    // Setup players from room
    let myIndex = 0;
    room.players.forEach((p: any, index: number) => {
      const isLocal = p.id === this.myId;
      if (isLocal) myIndex = index;
      
      const zPos = p.team === 'blue' ? -25 : 25;
      const xPos = (index % 4 - 1.5) * 6;
      const rotationY = p.team === 'blue' ? Math.PI : 0;
      ...
      if (isLocal) {
        this.camera.position.set(xPos, 1.6, zPos);
    ...
    // Initialize Balls from Server Room Data
    if (room.balls) {
      room.balls.forEach((b: any) => {
        this.addBall({
          id: b.id,
          position: b.position,
          velocity: b.velocity,
          ...
          lastInteractionTime: b.lastInteractionTime || Date.now()
        });
      });
    } else {
      // Fallback for safety
  ...
  private createHitEffect(position: { x: number, y: number, z: number }) {
    // Create a 3D "POW" effect
    const group = new THREE.Group();
    group.position.set(position.x, position.y, position.z);
    
    // Spikey sphere (Explosion)
    ...
    const animate = () => {
      ...
      const scale = 1 + (elapsed / 500) * 2;
      group.scale.set(scale, scale, scale);
      mesh.rotation.x += 0.1;
      mesh.rotation.y += 0.1;
      mat.opacity = 1 - (elapsed / 500);
      coreMat.opacity = 1 - (elapsed / 500);
  ...
  private setupSocketListeners() {
    ...
    this.socket.off('ball_thrown');
    this.socket.off('ball_grabbed');
    this.socket.off('player_eliminated');
    this.socket.off('game_over');
    this.socket.off('balls_update');
    ...
    this.socket.on('player_moved', (data) => {
      ...
      if (p && !p.data.isOut) {
        ...
        p.data.rotation = data.rotation;
        p.mesh.position.copy(data.position);
        p.mesh.rotation.y = data.rotation.y;
      }
    });

    this.socket.on('ball_thrown', (data) => {
      this.onBallThrown(data.ballId, data.position, data.velocity, data.id, data.type, data.curveFactor);
    });

    this.socket.on('ball_grabbed', (data) => {
      this.onBallGrabbed(data.ballId, data.id, data.isCatch);
    });

    this.socket.on('player_eliminated', (data) => {
      this.onPlayerOut(data.targetId, 'hit', data.shooterId);
      if (data.hitPosition) {
        this.createHitEffect(data.hitPosition);
      }
    });
    ...
    this.socket.on('balls_update', (ballsData: any[]) => {
      ballsData.forEach(data => {
        const ball = this.balls.get(data.id);
        if (ball) {
          // Only update if not held by us (to avoid jitter)
          const myPlayer = this.players.get(this.myId || '');
          if (myPlayer && myPlayer.data.holdingBallId === data.id) return;

          ball.data.position = data.position;
          ball.data.velocity = data.velocity;
          ball.data.state = data.state;
          ball.data.isLive = data.isLive;
          
          if (data.state !== 'held') {
            ball.mesh.position.copy(data.position);
    ...
    this.socket.on('timer_update', (timer: number) => {
  ...
  private addPlayer(data: PlayerData) {
    if (data.id === this.myId) {
      ...
      dummyMesh.visible = false; // Hide dummy mesh for self to avoid collision bugs
      this.players.set(data.id, { data, mesh: dummyMesh });
  ...
  private addBall(data: BallData) {
    ...
    // Apply current ball color to all balls for consistency
    if (this.currentBallColor) {
  ...
  private onBallGrabbed(ballId: number, playerId: string, isCatch: boolean = false) {
    const ball = this.balls.get(ballId);
    const player = this.players.get(playerId);
    if (ball && player) {
        ...
        if (thrower && thrower.data.team !== player.data.team) {
          ...
          if (playerId === this.myId) {
             ...
             if (lookDir.dot(toBall) < 0.8) {
               ...
               if (this.socket && ball.data.owner === this.myId) {
                 this.socket.emit('player_hit', { targetId: playerId });
               }
      ...
      if (playerId === this.myId) {
        ...
        if (this.socket) {
          this.socket.emit('ball_grab', { ballId });
        }
      }
    }
  }

  private eliminatePlayer(targetId: string, shooterId: string) {
    this.onPlayerOut(targetId, 'hit');
    const shooter = this.players.get(shooterId);
    const target = this.players.get(targetId);
    
    if (shooter && target) {
      shooter.data.score += 100;
      if (shooterId === this.myId) {
        this.onUpdateHUD({ score: shooter.data.score });
      }
      ...
      this.onUpdateHUD({ 
        kill: {
          id: Math.random(),
          shooterName: shooter.data.id === this.myId ? 'YOU' : (shooter.data.id.startsWith('bot') ? 'BOT' : 'PLAYER'),
          shooterTeam: shooter.data.team,
          targetName: target.data.id === this.myId ? 'YOU' : (target.data.id.startsWith('bot') ? 'BOT' : 'PLAYER'),
          targetTeam: target.data.team
        }
      });
    }
  }

  private onPlayerOut(playerId: string, reason: string, killerId?: string) {
    const player = this.players.get(playerId);
    if (player) {
      if (player.data.isOut) return; // Prevent duplicate processing
        ...
        if (killer) {
          this.onUpdateHUD({
            kill: {
              ...
              killer: killer.data.id === this.myId ? 'YOU' : killer.data.id,
              killerName: killer.data.id === this.myId ? 'YOU' : killer.data.name,
              killerTeam: killer.data.team,
              victim: player.data.id === this.myId ? 'YOU' : player.data.id,
              victimName: player.data.id === this.myId ? 'YOU' : player.data.name,
              victimTeam: player.data.team,
              isHeadshot: false
            }
          });
        }
  ...
  private checkWinCondition() {
    ...
    if (activeBlue === 0 || activeRed === 0 || this.roundTime <= 0) {
      if (this.socket) {
        // Server handles win condition online
        return;
      }
      ...
      console.log("Offline Game Over. Winner:", winner);

      // Generate fake scoreboard for offline mode
      const scoreboard = Array.from(this.players.values()).map(p => ({
        id: p.data.id,
        name: p.data.name,
        team: p.data.team,
        stats: {
          throws: Math.floor(Math.random() * 20),
          catches: Math.floor(Math.random() * 5),
          blocks: Math.floor(Math.random() * 5),
          eliminations: p.data.score // Use score as eliminations for now
        }
      }));

      this.onUpdateHUD({ winner, scoreboard });
      // Removed auto-restart to let UI handle it
    }
  }
  ...
  private animate() {
    ...
    this.emoteSprites.forEach((sprite, playerId) => {
      const player = this.players.get(playerId);
      if (player) {
        const pos = (playerId === 'local' || playerId === this.myId) ? this.camera.position.clone() : player.mesh.position.clone();
    ...
    if (this.fpShield) {
      const myPlayer = this.myId ? this.players.get(this.myId)?.data : null;
      this.fpShield.visible = this.isBlocking && (!myPlayer || !myPlayer.isOut);
      if (this.fpShield.visible) {
        (this.fpShield.material as THREE.MeshStandardMaterial).opacity = 0.3 + Math.sin(Date.now() * 0.01) * 0.1;
      }
    }
  ...
  private updateMovement(delta: number) {
    ...
    // Movement Logic
    let currentSpeed = 160.0; // Slightly buffed base speed since no sprint
    const isMoving = this.moveForward || this.moveBackward || this.moveLeft || this.moveRight || 
                     (this.isMobile && (Math.abs(this.mobileMove.x) > 0.1 || Math.abs(this.mobileMove.y) > 0.1));
    ...
    if (this.socket) {
      ...
      const pos = { x: this.camera.position.x, y: this.camera.position.y, z: this.camera.position.z };
      
      // Sync if moved or rotated significantly
      const moved = this.velocity.lengthSq() > 0.001;
      const rotated = Math.abs(rotationY - (this.lastSyncedRotation || 0)) > 0.01;
  ...
  private checkBallPlayerCollision(ball: any, id: number, delta: number) {
    ...
    this.players.forEach((player, playerId) => {
      ...
      const playerPos = (playerId === 'local' || playerId === this.myId) ? this.camera.position.clone() : player.mesh.position.clone();
      if (playerId !== 'local' && playerId !== this.myId) playerPos.y += 1.05; 
      else playerPos.y -= 0.5; 
      ...
      if (dist < 1.0) {
          // Check for block
          if (player.data.isBlocking || (playerId === this.myId && this.isBlocking)) {
            // Block successful!
            sounds.playBlock();
            ball.data.isLive = false;
            
            // Bounce ball back
            const bounceVel = new THREE.Vector3().subVectors(ball.mesh.position, playerPos).normalize().multiplyScalar(15);
            ball.data.velocity.x = bounceVel.x;
            ball.data.velocity.y = bounceVel.y;
            ball.data.velocity.z = bounceVel.z;
            ball.data.state = 'idle';
            ball.data.owner = null;

            if (playerId === this.myId && this.socket) {
              this.socket.emit('player_block');
            }
            return;
          }

          sounds.playHit();
          this.onPlayerOut(playerId, 'hit', ball.data.owner || undefined);
  ...
  private updateBots(delta: number) {
    if (this.socket) return; // No bots in online mode
  ...
  private updateBalls(delta: number) {
    if (this.socket) {
      // In online mode, we still need to update trails and check for local collisions if we are the owner?
      // Actually, server handles hits now too? Let's check server.ts.
      // Server doesn't handle hits yet. I should add hit detection to server or keep it on client.
      // User said: "ball movement all should be calculated on server so everyone see the same thing"
      // I'll keep hit detection on client for now as it's more responsive, but server manages positions.
      ...
      this.balls.forEach((ball, id) => {
        ...
        // Client-side hit detection (still needed for responsiveness, server can validate)
        if (ball.data.state === 'thrown' && ball.data.isLive) {
          this.checkBallPlayerCollision(ball, id, delta);
        }
      });
      return;
    }
